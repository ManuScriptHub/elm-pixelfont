...
validCode : Char -> Int
validCode c =
    (min toCode <| max (Char.toCode c) fromCode) - fromCode


charToBinaryStrings : Char -> List String
charToBinaryStrings code =
    List.getAt (validCode code) asciiChars
        |> Maybe.withDefault (List.repeat 13 "00")
        |> List.map (Result.extract (\_ -> 0) << ParseInt.parseIntHex)
        |> List.map (\b -> Result.extract (\_ -> "0") <| ParseInt.toRadix 2 b)
        |> List.map (String.padLeft 8 '0')
        |> List.reverse


replaceChars : ( Char, Char ) -> List String -> List String
replaceChars ( zero, one ) strings =
    List.map (\s -> replaceAll ( "0", zero ) s |> replaceAll ( "1", one )) strings


stringToGrids : String -> List Grid
stringToGrids str =
    String.toList str |> List.map (List.indexedMap row << charToBinaryStrings)


replaceAll : ( String, Char ) -> String -> String
replaceAll ( search, repl ) input =
    replace Regex.All (regex search) (\_ -> String.fromChar repl) input


row : Int -> String -> List (Maybe ( Int, Int ))
row row str =
    List.indexedMap (cell row) <| String.toList str


cell : Int -> Int -> Char -> Maybe ( Int, Int )
cell col row c =
    case c of
        '1' ->
            Just ( col, row )
...
--source: http://stackoverflow.com/questions/2156572/c-header-file-with-bitmapped-fonts
-- space :32
-- ! :33
-- :126


asciiChars : List (List String)
asciiChars =
